...
async function fetchAllOrders(filter) {
  let query = supabase
    ...
    .select(`
      ...
      order_items (
        quantity, unit_price,
        books!order_items_book_id_fkey ( title, author, cover_url, slug )
      )
    `)
...
function OrderRow({ order, onStatusChange }) {
  ...
  const client = order.profiles?.full_name
    || order.profiles?.email
    || 'Invitado'
  // Línea del itemCount — era i.qty:
    ...
    <>
      ...
      <tr style={{ cursor: 'pointer' }} onClick={() => setOpen(o => !o)}>
        ...
        <td>
          ...
          {order.guest_email && (
            <span className="badge badge-gray" style={{ fontSize: 10, marginTop: 2 }}>Invitado</span>
          )}
        </td>
        ...
        <td style={{ fontWeight: 700, color: 'var(--jv-brown-700)' }}>
        ...
        <td style={{ color: 'var(--jv-ink-muted)', fontSize: 12 }}>
          {open ? '▲' : '▼'}
        </td>
      </tr>
            ...
            <div style={{
                  ...
                  <div key={i} style={{
                    ...
                    {item.books?.cover_url ? (
                      <img
                        ...
                        style={{ width: 36, aspectRatio: '3/4', objectFit: 'cover', borderRadius: 4, flexShrink: 0 }}
                      />
                    ) : (
                      <div style={{
                        width: 36, aspectRatio: '3/4', background: 'var(--jv-brown-200)',
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ margin: 0, fontSize: 11, color: 'var(--jv-ink-muted)' }}>
                        {item.books?.author} - x{item.quantity} - Bs. {Number(item.unit_price).toFixed(2)} c/u
                      </p>
                    </div>
                    <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--jv-brown-700)' }}>
                      Bs. {(Number(item.unit_price) * item.quantity).toFixed(2)}
                    </span>
                  </div>
                ))}
              </div>
              ...
              {order.shipping_address && (
                <div style={{
                  ...
                  borderTop: '1px dashed var(--jv-border)',
                  display: 'grid',
                  ...
                  <div>
                    <p style={{ margin: 0, fontWeight: 600, color: 'var(--jv-ink-soft)', marginBottom: 6 }}>
                      📦 Datos de entrega
                    </p>
                    <p style={{ margin: 0, color: 'var(--jv-ink-muted)' }}>
                  ...
                  <div>
                    ...
                    {order.shipping_address.notes && (
                      <p style={{ margin: 0, color: 'var(--jv-ink-muted)', fontStyle: 'italic' }}>
                        💬 {order.shipping_address.notes}
                      </p>
                    )}
                  </div>
              ...
              <div style={{
                marginTop: 12, paddingTop: 12, borderTop: '1px dashed var(--jv-border)',
                display: 'flex', justifyContent: 'flex-end', fontSize: 14,
              }}>
                <span style={{ color: 'var(--jv-ink-muted)', marginRight: 12 }}>Total</span>
                <span style={{ fontWeight: 700, color: 'var(--jv-brown-700)' }}>
                  Bs. {Number(order.total).toFixed(2)}
                </span>
              </div>
            </div>
  ...
  return (
    <AdminLayout title="Pedidos">
      ...
      <p style={{ fontSize: 13, color: 'var(--jv-ink-muted)', marginBottom: 16 }}>
        ...
        {filter !== 'all' ? ` · filtro: ${STATUS_OPTIONS.find(s => s.value === filter)?.label}` : ''}
      </p>
