...
async function fetchGenres() {
  const { data, error } = await supabaseData.from('genres').select('id, name').order('name')
  if (error) throw error
  return data
}
...
function BookModal({ book, genres, onClose }) {
  ...
  async function handleImageUpload(file) {
    ...
    try {
      ...
      const { error: upError } = await supabaseData.storage
        .from('book-covers')
        .upload(path, file, { contentType: file.type })

      if (upError) throw upError

      const { data } = supabaseData.storage.from('book-covers').getPublicUrl(path)
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (isEdit) {
        const { error } = await supabaseData.from('books').update(payload).eq('id', book.id)
        if (error) throw error
      } else {
        const { error } = await supabaseData.from('books').insert(payload)
        if (error) throw error
      }
    },
    onSuccess: () => {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="input-group">
            ...
            <input className="input" value={form.slug}
              onChange={e => set('slug', slugify(e.target.value))} placeholder="mi-libro-titulo" />
          ...
          <div className="input-group">
            ...
            <textarea className="textarea" rows={4} value={form.description}
              onChange={e => set('description', e.target.value)} placeholder="Sinopsis del libro…" />
          </div>
          ...
          {mutation.isError && (
            <div style={{ background: 'var(--jv-error-bg)', color: 'var(--jv-error)', padding: '10px 14px', borderRadius: 'var(--radius-md)', fontSize: 13 }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', paddingTop: 8 }}>
            ...
            <button className="btn btn-primary" onClick={handleSubmit} disabled={mutation.isPending || uploading}>
              {mutation.isPending
...
export default function AdminBooks() {
  ...
  async function handleDelete(book) {
    const { error } = await supabaseData.from('books').delete().eq('id', book.id)
    if (!error) {
      queryClient.invalidateQueries({ queryKey: ['admin-books'] })
      queryClient.invalidateQueries({ queryKey: ['admin-metrics'] })
    }
  ...
  return (
    <AdminLayout title="Libros">
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        <input className="input" placeholder="Buscar por título o autor…" value={search}
          onChange={e => setSearch(e.target.value)} style={{ flex: '1 1 240px' }} />
        <button className="btn btn-primary" onClick={() => setModal('create')}>+ Nuevo libro</button>
      </div>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div className="table-wrapper">
            <table className="table">
              <thead>
                <tr>
                  <th>Portada</th><th>Título / Autor</th><th>Género</th>
                  <th>Precio</th><th>Stock</th><th>Destacado</th><th>Acciones</th>
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 && (
                  <tr><td colSpan={7} style={{ textAlign: 'center', padding: 28, color: 'var(--jv-ink-muted)' }}>
                    {search ? 'Sin resultados.' : 'No hay libros aún. ¡Crea el primero!'}
                  </td></tr>
                )}
                {filtered.map(book => (
                  <tr key={book.id}>
                    <td>
                      {book.cover_url ? (
                        ...
                        <div style={{ width: 36, aspectRatio: '3/4', background: 'var(--jv-brown-100)',
                          borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>📖</div>
                      )}
                    </td>
                    <td>
                    ...
                    <td style={{ fontWeight: 600, color: 'var(--jv-brown-700)' }}>Bs. {Number(book.price).toFixed(2)}</td>
                    <td>
                      <span className={`badge ${book.stock === 0 ? 'badge-red' : book.stock <= 5 ? 'badge-yellow' : 'badge-green'}`}>
                        {book.stock}
                      </span>
                    </td>
                    <td style={{ textAlign: 'center' }}>{book.featured ? '⭐' : '—'}</td>
                    <td>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button className="btn btn-ghost btn-sm" onClick={() => setModal(book)}>Editar</button>
                        <button className="btn btn-ghost btn-sm" style={{ color: 'var(--jv-error)' }}
                          onClick={() => setDelTarget(book)}>Eliminar</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {modal && <BookModal book={modal === 'create' ? null : modal} genres={genres} onClose={() => setModal(null)} />}
      {delTarget && <ConfirmDelete book={delTarget} onConfirm={() => handleDelete(delTarget)} onCancel={() => setDelTarget(null)} />}
    </AdminLayout>
  )
}